...
- **思考レベル(`config.thinkingConfig.thinkingLevel`)は既定のまま**にしてある。Gemini 3.x は既定で思考するぶん遅く・高くなるが、実測 10〜12秒で要件(通常15秒以内)を満たし、実物のレシートで精度も確認できたため触っていない。**実運用で遅いと感じたら `LOW` / `MINIMAL` に下げる**のが最初の一手(Claude側の `effort: "low"` に相当。精度とのトレードオフなので、同じレシートで前後を比べること)
...
- [x] `convex/receipts.ts`(**ファイル先頭に `"use node";`** — Anthropic SDKを使うため)。action `parse`:
  ...
  6. **AI由来の品目数(`sourceItemCount`)が0なら**「レシートを読み取れませんでした」(レシート以外・不鮮明な画像)。成功ログもこの判定の後に出す
- [x] `lib/receipt.ts`(**純粋関数。AI呼び出しと切り分けてテストする**。`lib/settlement.ts` と同じ方針):
  - AIの返り値を `expenses.save` の制約(V-402 / V-403)に均す(名前50字・金額1〜9,999,999の整数・数量1〜999、保存できない行は捨てる、品目は100件まで)
  ...
  - AIが `price` を単価で返してしまった場合の救済: 行合計として足すと `total_amount` に合わず、`price × quantity` なら一致するときは単価とみなして行合計に直す(どちらとも判定できなければ指示どおり行合計として扱い、ズレは差額の配分が吸収する)
  - 品目合計 ≠ `total_amount` のとき、差額を**各品目へ金額比で配分**する(`distributeDifference`)。端数は「累積の目標値との差」で決めるので、配分後の品目合計は必ず `total_amount` と一致する
  - ⚠️ **配分すると1円未満に潰れる品目が出るときは配分しない**。金額は「1円以上9,999,999円以下の整数」(V-403)で、そのままでは `expenses.save` が必ず弾くため。代わりに `distributionSkipped: true` を返し、画面が「金額を確認してください」と促す
  - 📌 **当初は差額を「調整(税・割引等)」1行にまとめる実装だった**(要件の初版)。税別レシートの実運用でこの行の負担区分が既定の折半になり、個人品目に掛かった税まで折半されてしまうことが分かったため、TBD-003 の代替案「金額比の自動按分」へ変更した(2026-08-02)。詳細と既知の限界(軽減税率の混在)は `docs/requirements.md > TBD-003 の決着` を参照
...
- 品目合計と合計金額がズレるレシート(税別レシートなど)で、差額が各品目に配分され、品目合計がレシートの合計金額と一致する
...
> ✅ **実地確認の結果(2026-07-27)**: `GEMINI_API_KEY` 登録後、合成レシートと**実物のレシート**の両方で読み取り成功。所要時間は **10〜12秒**(要件の「通常15秒以内」を満たす)。店名・購入日・品目・金額が自動入力され、消費税ぶんが「調整(税・割引等)」として自動追加されることを確認。既定モデル `gemini-3.6-flash` は実際に動作する。
>
> このとき初めて分かった不具合を2件修正した(どちらもユニットテストでは出ない):
> 1. **購入日が常に当日になっていた** — AIに渡すスキーマ/プロンプトに日付の形式を書いておらず、「2026年7月21日」の形で返ると静かに捨てられていた。フィールドの説明は**コメントではなく `.describe()`** で持たせること(JSON Schema に載ってAIに渡る)
> 2. **数量が「牛乳 x2 ×2」と二重に付いた** — AIは数量を品目名に含めたり含めなかったりする。いったん外して付け直す
