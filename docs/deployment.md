...
>
> 値はカンマ区切りの**1件ずつ**処理される。間違え方は2通りあり、症状が逆になる:
> - **本番のオリジンが1件も含まれていない**(タイポ・`http`/`https` の取り違えだけを書いた)
>   → **全員ログインできなくなる**
> - **オリジンとして解釈できない値を書いた**(`example.com` のようにスキーム無し、`ftp://` など)
>   → その値だけ `proxy.ts` が無視する。有効なオリジンが1件でも残っていれば検査は続き、
>   **1件も残らなかったときだけ検査なしに倒れる**(=保護が掛からない。ログに警告が出る)
>
