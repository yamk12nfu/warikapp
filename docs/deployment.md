...
>
> 間違え方は2通りあり、症状が逆になる:
> - **別のオリジンを書いた**(タイポ・`http`/`https` の取り違え)→ **全員ログインできなくなる**
> - **オリジンとして解釈できない値を書いた**(`example.com` のようにスキーム無し、`ftp://` など)
>   → `proxy.ts` が無視して**検査なしに倒れる**(=保護が掛からない。ログに警告が出る)
>
...
| `CLERK_AUTHORIZED_PARTIES` を入れたのに効いていない | 値が http(s) のオリジンとして解釈できず、`proxy.ts` に無視された(`example.com` のようにスキームが無い、`ftp://` などの別スキーム)。**このときは締め出しではなく「検査なし」に倒れる** | §3-4。Vercelの実行ログに `CLERK_AUTHORIZED_PARTIES: …を無視しました` が出る |
