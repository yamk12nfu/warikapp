...
>
> 値はカンマ区切りの**1件ずつ**処理される。結果は残ったオリジンの中身で決まる:
>
> | 書いた値 | `proxy.ts` が渡すもの | 症状 |
> |---|---|---|
> | 本番オリジンを正しく書いた | そのオリジン | 正常 |
> | http(s)のオリジンとしては正しいが**本番と別のもの**(タイポ・`http`/`https` の取り違え) | その別オリジン | **全員ログインできなくなる** |
> | オリジンとして解釈できない値だけ(`example.com` のようにスキーム無し、`ftp://`、`,` だけ) | **何も渡さない** | 検査なしに倒れる(保護が掛からない。ログに警告) |
> | 正しい本番オリジン + 解釈できない値 | 正しいオリジンだけ | 正常(無視した値をログに警告) |
>
