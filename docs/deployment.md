...
> | 1件以上あるが本番オリジンを含まない | その別オリジン | **全員ログインできなくなる** |
> | 0件 | **何も渡さない** | 検査なしに倒れる(保護が掛からない) |
>
> 解釈できなかった値は、残った件数にかかわらず1件ずつログに警告が出る。
> 残りが0件になったときは、それに加えて「検査を行いません」の警告が出る。
> **「1件以上あるが本番を含まない」だけは警告が出ない**(値としては正しいので、
> `proxy.ts` からは間違いだと分からない)。全員ログインできなくなったらここを疑う。
>
