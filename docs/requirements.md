...
| expenses | 支出(coupleId, paidBy, storeName, purchasedAt, totalAmount, imageStorageId, source: receipt/manual, status: draft/confirmed, settlementId, deletedAt, fixedCost: {id, month}?)。品目リスト items[] を内包し、各品目が name / price / quantity / shares[](memberId, ratioPercent ※2名合計100)を持つ |
