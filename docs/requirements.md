...
- 固定費テンプレートを登録・編集し、当月分を毎月自動計上
- 月ごとの支出合計・二人の支払/負担額・分類別集計・負担の推移の表示
...
| US-011 | パートナー | 家賃などを繰り返し入力しないため、固定費を登録して毎月の支出に自動計上したい | - 固定費を今月分または来月分から登録できる<br>- 計上済みの月は二重登録されず、削除しても自動で復元しない<br>- 固定費テンプレートを停止できる | Should |
...
| F-008 | 仕分けのAI提案 | 過去の仕分け実績をもとに品目の負担区分を初期提案。第一段階(履歴ベース)は実装済み、AIによる提案は未着手 | API | Should | US-002 | 概要のみ |
| F-009 | 月次サマリー・カテゴリ集計 | 月ごとの支出合計・二人の支払/負担額・分類別集計・直近6か月の負担の推移(`/months`) | UI, API | Could | — | 概要のみ |
| F-010 | PWA/iOSアプリ化 | ホーム画面追加、ストア公開 | UI | Could | US-010 | 概要のみ |
| F-011 | 固定費 | 固定費テンプレートを登録し、当月分を毎月支出として自動計上 | UI, API, Cron | Should | US-011 | 詳細 |

### 3.2 機能詳細(Must機能・詳細記載機能)
...
4. 世帯から退出: パートナーがいる場合は未精算の確定支出と下書きがないことを確認 → 自分のメンバー行を退出済みにして世帯から外れる。精算済みの支出・精算履歴は残り、相手がいなくなった場合は世帯データとレシート画像を削除する
5. アカウント削除: 世帯から退出 → Clerkのユーザーをサーバー側で削除 → セッションを終了する。世帯から退出済みでClerk削除に失敗した場合は、`/setup` から再試行する
...
| V-204 | 世帯から退出 | パートナーがいる場合、未精算の確定支出または下書きが残っていないこと(1人の世帯では退出と同時に世帯が消えるため対象外) | 残っていれば退出を拒否し、先に精算・確定または削除を促す |
...
| items[].price | int | 必須 | レシートに印字された金額(円)。税抜表記ならそのまま税抜。値引きはその品目に反映 |
...
4. 応答JSONをスキーマ検証。品目合計 ≠ total_amount の場合、差額(税別レシートの消費税、総額からの値引きなど)を**各品目へ金額比で配分**する。配分後の品目合計は total_amount と必ず一致する。配分すると1円未満に潰れる品目が出る場合のみ配分せず、画面で金額の確認を促す
...
| 店名 | string | No | 50文字以内 | |
| 購入日 | date | Yes | 未来日不可 | |
| 分類 | enum | No | 食費 / 日用品 / 交通 / 住居・光熱 / 医療 / 娯楽 / その他 | 支出単位(品目単位ではない)。未選択は「未分類」として保存時にフィールドを省略 |
...
- 初期値: 全品目「折半」。レシート読み取り時は、過去に確定した同じ品目名があればその負担区分を初期値にし「前回」印を付ける(F-008 第一段階)
...
**概要**: 家賃や光熱費などを固定費テンプレートとして登録し、対象月の支出を `expenses` に自動作成する。テンプレートと計上済みの支出は世帯単位で管理する。
...
| 項目名 | 型 | 必須 | バリデーション | 備考 |
|--------|-----|------|--------------|------|
| 名目 | string | Yes | 1〜50文字 | 前後の空白を除去して保存する |
| 金額 | int | Yes | 1〜9,999,999円 | 円単位・整数。V-403を再利用 |
| 支払者 | member_id | Yes | 現在の世帯メンバー | |
| 負担区分 | share[] | Yes | 合計100%、メンバー重複なし | V-401を再利用 |
| 分類 | stored category または null | Yes | 既存の分類または未分類 | 未分類は保存時に分類フィールドを省略 |
| 開始月 | `this` / `next` | 作成時Yes | 今月分または来月分 | 既定値は今月分。編集時は変更しない |

**処理フロー**:
1. `/fixed-costs` から固定費を追加し、名目・金額・支払者・負担区分・分類を入力する。作成時は「今月分から」(既定)または「来月分から」を選ぶ。今月分を選ぶと登録時にすぐ支出へ計上される。
2. 保存時はログイン中のメンバーと世帯を確認し、名目・金額・負担区分と支払者/負担メンバーの世帯所属を検証する。編集では停止済みテンプレートを変更できない。
3. 保存の最後に当月分の計上を試みる。対象月の支出が既にあれば変更せず、なければテンプレートの名目・金額・支払者・負担区分・分類を使って支出を1件作る。支出は `confirmed`、`source: "manual"`、`purchasedAt` は対象月の1日とする。
4. 毎日JST 00:05(UTC 15:05)に、全世帯の稼働中テンプレートから当月分だけを計上する。日次実行なので月内の一時的な失敗は翌日以降に再試行される。月末まで計上されなかった月は自動補完せず、一覧の「未計上」を見て手入力する。
5. 一覧は稼働中と停止済みを分け、当月の状態を「計上済み」「削除済み」「未計上」「開始前」で表示する。計上履歴は直近12件まで表示する。
6. テンプレートの編集は計上済み支出へ反映しない。対象月が未計上なら保存時に当月分の計上を試みる。停止は冪等で、既存支出を変更・削除しない。再開はできないため、新しい固定費として登録する。
7. 計上済み支出を削除しても、固定費IDと対象月の組み合わせを計上済みの印として残し、再計上しない。利用者が購入日を編集しても `fixedCost.month` は変えない。
8. 支払者または負担区分に含まれるメンバーの退出を次回計上時に検知した場合、その固定費を `memberLeft` 理由で自動停止する。

**バリデーションルール**:
| ルールID | 対象項目 | ルール | エラー時の振る舞い |
|---------|---------|--------|-----------------|
| V-1101 | 名目 | 前後空白を除去した後、1〜50文字 | 「名目は1〜50文字で入力してください」 |
| V-1102 | 開始月 | 作成時は `this` または `next`。既定は `this` | 入力を限定し、今月分/来月分のどちらかを選ぶ |
| V-1103 | 更新 | 停止済みテンプレートは変更不可 | 「停止した固定費は変更できません」 |

負担割合はV-401(金額割合合計100%、重複なし)、金額はV-403(1〜9,999,999円の整数)の規則とエラーメッセージを再利用する。支払者と負担メンバーは現在の世帯に所属している必要がある。

**エラーハンドリング**:
| エラー条件 | エラーメッセージ | 振る舞い |
|-----------|----------------|---------|
| 他世帯・不存在・不正な固定費ID | 「固定費が見つかりません」(詳細取得は `null`) | 同じ扱いにして存在を漏らさない |
| 他世帯または退出済みメンバーを指定 | 「権限がありません」 | 保存を拒否する |
| 名目・金額・負担割合が不正 | V-1101またはV-403/V-401のメッセージ | 保存を拒否し、画面に表示する |
| 停止済みテンプレートを編集 | 「停止した固定費は変更できません」 | 保存を拒否する |
| 当月分が未計上 | 「未計上」 | 自動補完せず、手入力を案内する |

**データ永続化**: `fixedCosts` に世帯ID、名目、金額、支払者、負担区分、分類、開始月、停止日時、停止理由を保存する。`expenses.fixedCost` は `{ id, month }` の組でテンプレートと対象月を示し、同じ組の既存行を論理削除済みも含めて検索する。計上行は通常の支出と同じく精算・編集・削除できる。世帯の最後のメンバーが退出したときは、支出削除の後に固定費テンプレートもバッチ削除する。

### 3.3 機能概要(Should/Could機能)

#### F-008: 仕分けのAI提案(Should)
**概要**: 過去の仕分け実績をもとに品目ごとの負担区分の初期値を提案し、ユーザーは確認画面で修正するだけにする(例: 「ビール」→過去実績から「自分」)。
**第一段階(実装済み、履歴ベース)**: レシート読み取り後、世帯の未削除支出を購入日の新しい順に100件読み、確定済み支出の品目のうち品目名(正規化キー)が一致する直近の負担区分を初期値にする(`convex/expenses.ts` の `suggestReceiptItemShares`、`lib/share-memory.ts`)。一致しない品目は折半。一致した品目は確認画面で「前回」印を表示し、負担区分を手で変えるか品目名を編集すると印が消える。提案の取得に失敗しても読み取りは続行し、全品目折半で確認画面へ進む。手入力(F-005)では提案しない。
**今後(未着手)**: 類似品目や表記ゆれを含めたAIによる提案。想定される入出力: 入力=品目リスト+過去の仕分け実績、出力=品目ごとの推奨負担区分と確信度。

#### F-009: 月次サマリー・カテゴリ集計(Could)
**概要**: 月ごとの支出を `/months/[YYYY-MM]` で集計して表示する(`/months` は当月へリダイレクト)。ヘッダーの「月次」から遷移し、前の月・次の月へ移動できる(当月より先へは進めない)。集計対象は未削除かつ確定済みの支出で、ドラフトは含めない。
**表示内容**: この月の合計と未精算額、二人それぞれの支払額と負担額、分類別の金額と割合バー(金額0の分類は出さず、未分類は末尾)、直近6か月の負担の推移(各月のリンク付き)。1か月の支出が200件を超えると「支出が多すぎてこの月は集計できません」と表示して集計しない。
**実装**: 集計は `lib/month-book.ts` の純粋関数、読み出しは `convex/monthBook.ts` の `monthBook.month`。

#### F-010: PWA/iOSアプリ化(Could)
**概要**: まずPWA(ホーム画面追加・カメラ起動)対応、その後Capacitor等でのストア公開を検討。認証はClerkのままApple providerを追加。第一段階としてmanifest・アイコン・standalone対応を実装済みで、サービスワーカー・プッシュ通知は今後の対応とする。
...
| S-010 | 固定費一覧 | 稼働中/停止済みテンプレートと当月状態を表示 | /fixed-costs |
| S-011 | 固定費登録 | 固定費の入力と開始月の選択 | /fixed-costs/new |
| S-012 | 固定費詳細 | テンプレート編集、停止、計上履歴 | /fixed-costs/[id] |
| S-013 | 月次サマリー | 月の合計・二人の支払/負担・分類別集計・負担の推移 | /months, /months/[YYYY-MM] |
...
graph TD
    ...
    S003 --> S009[S-009 設定]
    S003 -->|固定費| S010[S-010 固定費一覧]
    S010 -->|追加| S011[S-011 固定費登録]
    S011 -->|登録| S010
    S010 -->|固定費タップ| S012[S-012 固定費詳細]
    S012 -->|支出履歴| S005
    S012 -->|更新保存| S012
    S012 -->|停止| S010
    S008 --> S003
...
| expenses | 支出(coupleId, paidBy, storeName, purchasedAt, totalAmount, category?, imageStorageId, source: receipt/manual, status: draft/confirmed, settlementId, deletedAt, fixedCost: {id, month}?)。品目リスト items[] を内包し、各品目が name / price / quantity / shares[](memberId, ratioPercent ※2名合計100)を持つ |
| fixedCosts | 固定費テンプレート(coupleId, name, amount, paidBy, shares, category, startMonth, stoppedAt, stoppedReason) |
| settlements | 精算(coupleId, fromMemberId, toMemberId, amount, memo, settledBy) |
| uploads | アップロード画像の世帯帰属台帳(coupleId, storageId, uploadedBy, usedByExpenseId?)。読み取り・保存時に自世帯の画像かを検証し、未参照の画像は撮り直し時に破棄できる |

レート制限(AI読み取り30回/時/世帯、アップロードURL発行60回/時/世帯)は `@convex-dev/rate-limiter` コンポーネントが自前のテーブルで持つため、上記には含めない。
...
| ~~TBD-003~~ | F-003 | ~~税・割引の「調整行」方式(デフォルト折半)で実運用上問題ないか。代替案: 品目金額比での自動按分~~ **決着(2026-08-02)**: 税別レシートの運用で問題が出たため代替案(金額比の自動按分)へ変更。理由は下記 | 二人で運用確認 | 完了 |
| TBD-004 | F-010 | iOSアプリ化の方式(PWA止まり / Capacitor / ネイティブ)と時期 | 開発者 | 公開検討時 |
| TBD-005 | F-008 | ~~仕分けAI提案の実装時期(MVP後の最初の改善候補)~~ **一部決着**: 履歴ベースの提案を第一段階として実装済み(F-008参照)。残りはAIによる類似品目・表記ゆれの提案で、実装時期は未定 | 開発者 | 未定 |
| TBD-006 | 6.1 | Gemini切り替え時のプロンプト・スキーマ互換性検証(構造化出力の指定方法がプロバイダ間で異なる) | 開発者 | 切り替え時 |
| ~~TBD-007~~ | F-002 | ~~世帯からの退出・世帯削除・アカウント削除のフロー詳細(MVPでは設定画面に最小限の導線のみ)~~ **決着(2026-09-23)**: 下記のとおり実装 | 開発者 | 完了 |
| TBD-008 | F-011 | cronで取りこぼした月の補填方法(自動backfill/手入力)、任意支払日、停止済み固定費の再開方法 | 開発者 | 運用開始後 |

#### TBD-003 の決着(2026-08-02)

税別レシート(品目が税抜表記、合計だけ税込)を実運用したところ、調整行方式に2つの問題が出た。

1. **手直しが毎回発生する** — 差額(消費税ぶん)が「調整(税・割引等)」1行になり、その負担区分が既定の折半になる。片方の個人品目(酒・化粧品など)が金額の大半を占めるレシートでは、その品目に掛かった税まで折半されてしまうため、毎回この行の負担区分を手で変える必要があった
2. **正しい値を判断できない** — 1行にまとまっているので「いくらを誰に寄せれば正しいのか」がユーザーには分からず、直すには自分で按分を計算するしかなかった

代替案の**金額比による自動按分**に切り替えた。差額を各品目の金額比で配分して品目価格に上乗せするので、税は**その品目の負担区分にそのまま従う**(酒が自分100%なら酒の税も自分100%)。手直しも判断も不要になる。

**既知の限界**: 金額比の按分は税率が一律であることを前提とするため、軽減税率(8%)と標準税率(10%)が混在するレシートでは、食品側に税が数円〜数十円多く乗る。「税を全部折半」だった従来より不公平は小さいが、完全に正確ではない。正確にするにはAIに品目ごとの税率も返させて税率グループごとに按分する必要があり、AIスキーマ・プロンプトの変更を伴うため見送った(必要になれば再検討する)。

#### TBD-007 の決着(2026-09-23)

退出前に世帯全体の未精算支出を確認し、確定済みの未精算支出だけでなく下書きも残っていれば退出を拒否する。退出したメンバー行は削除せず、認証との紐付けだけを外して表示名を精算履歴に残す。最後の現役メンバーが退出した時点で、支出・画像台帳・精算履歴・招待・メンバー・世帯行をバッチで削除し、レシート画像もConvex File Storageから削除する。アカウント削除は「世帯から退出してからClerkユーザーを削除する」順序とし、Clerk側の削除に失敗した場合は世帯未所属の`/setup`から再試行できる。
