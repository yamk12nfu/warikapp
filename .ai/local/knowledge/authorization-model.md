...
- **UI層のリソースレベル認証**: 保護ページは冒頭で `requireSignedIn()`(`lib/server-auth.ts`)を
  呼ぶ(Clerk推奨。layoutはクライアント遷移で再実行されないことがあるため、layout/proxyは
  追加防御でありページ側チェックを省略しない)。動的セグメントを持つルートを追加したら
  `proxy.ts` のmatcherにも明示追加する
